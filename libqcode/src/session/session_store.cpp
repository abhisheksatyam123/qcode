...
void reload_session_history(const std::string& session_id, ChatState& state) {
    ...
    // Live counters belong to the previous session — clear them, then restore
    // the persisted cumulative totals so the Stats tab shows the real numbers
    // for the freshly opened session instead of 0.
    ...
    // NOTE: tool_call_count / total_tool_time_ms are intentionally NOT cleared
    // here anymore — their persisted cumulative values are restored just above
    // (reconstructed from stored messages) so the Stats tab survives a session
    // switch instead of dropping to 0.
